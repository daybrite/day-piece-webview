...
Verified rendering + two-way URL binding + controls on **AppKit, Qt, UIKit (iOS sim), Android**. **GTK
and WinUI are written blind** (no WebKitGTK / Windows host on the reference machine) to build + run in
CI — the GTK `webkit6` API is verified against the crate source, and both are captured in the CI gallery.
...
1. **Android manifest permissions.** A web view needs `INTERNET`, but a piece can't edit the app manifest.
   ...
   generated overlay manifest that AGP merges into the app manifest — zero app edits.
