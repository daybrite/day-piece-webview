...
registers `day.webview.eval` with day-core through `register_piece_operation`, keyed by its node
kind, and the step drives it through `day_core::piece_operation`; day-script and the
...
resolve by looking the id up in a thread-local map, and follow [`docs/async.md`](https://github.com/daybrite/day/blob/main/docs/async.md)'s rule that a piece
